...
class Conv2D(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        #shape kernel_size + inputshape[-1] + filters_num
        #self.kernel = self.kernel_initializer(shape=(self.kernel_size+(input_shape[-1],self.filters)))
        self.kernel = self.kernel_initializer(shape=(self.kernel_size[0],self.kernel_size[1],input_shape[-1],self.filters))
        self.bias = self.bias_initializer(shape=(self.filters,))#(num_filters,1)
        return None
...
class BatchNormalization(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        #init beta,gamma,mov_mean, mov_vari by initializer
        self.moving_mean = tf.Variable(self.moving_mean_initializer(shape=input_shape[self.axis]),trainable=False)
        self.moving_var = tf.Variable(self.moving_variance_initializer(shape=input_shape[self.axis]),trainable=False)

        if self.center:
            self.beta = self.beta_initializer(shape=1)
            self.beta =  tf.Variable(self.moving_mean,trainable=True)

        if self.scale:
            self.gamma = self.gamma_initializer(shape=1)
            self.gamma =  tf.Variable(self.moving_var,trainable=True)
        
        ## TODO 2: Initialize components as tf.Variables; specify trainable status as appropriate
        #tf.Variables() load all some are trainable some not(beta&gamma)
        #mov_mean, mov_vari not train
    ...
    def call(self, inputs, training=False):
        ...
        if training:
            ...
            self.moving_var.assign(self.moving_var * self.momentum + bvar *(1-self.momentum))
        
            outputs = (inputs - bmu) / tf.sqrt(bvar + self.epsilon)
        else:
            outputs = (inputs - self.moving_mean) / tf.sqrt(self.moving_var + self.epsilon)
            #outputs = self.gamma * (inputs - bmu)/tf.sqrt(bvar+self.epsilon)+self.beta
            #outputs = (inputs - bmu)/tf.sqrt(bvar+self.epsilon)
        
            #mov_mean.assign(calculated value) assgin update so mov_var
            #outputs
        #else:
            #not update mov~, output = xxxx
       # else:
           # outputs = self.gamma *(inputs - self.moving_mean)/tf.sqt(self.moving_var + self.epsilon) + self.beta
...
class Dropout(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training=False):
        ...
        #assert 0 <= dropout <= 1
        if training:
            #if self.noise_shape: ##== 1: return tf.zeros_like(X)
            #if self.noise_shape is None:
                do = tf.greater_equal(tf.random.uniform( shape=tf.shape(inputs)),self.rate)#< 1 - self.rate
            #else:
                #mask= tf.random.uniform( self.noise_shape, minval=0, maxval=1)<1-self.rate
        
                return tf.cast(do, dtype=tf.float32) * inputs / (1.0 - self.rate)
